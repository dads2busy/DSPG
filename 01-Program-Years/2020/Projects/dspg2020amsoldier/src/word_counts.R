...
data1 <- dbGetQuery(conn, "SELECT *
                   FROM american_soldier.survey_32_clean");

data2 <- dbGetQuery(conn, "SELECT *
                    FROM american_soldier.survey_32");
# disconnect from postgresql
dbDisconnect(conn)

# do process for data1
# split into racial groups
S32N <- data1 %>% filter(racial_group == "black")
S32W <- data1 %>% filter(racial_group == "white")
...
# remove NAs
s32_clean <- s32 %>% filter(!is.na(text))

# do process for data2
# split into racial groups
S32N <- data2 %>% filter(racial_group == "black")
S32W <- data2 %>% filter(racial_group == "white")
...
# remove NAs
s32_not_clean <- s32 %>% filter(!is.na(text))

# get word counts
# for not clean
s32_not_clean_tokens <- s32_not_clean %>%
  unnest_tokens(word, text)
length(s32_not_clean_tokens$word) # 381,339
# for clean
s32_clean_tokens <- s32_clean %>%
  unnest_tokens(word, text)
length(s32_clean_tokens$word) # 11,162
