...
source("theme.R")
#Get Data
Tree<-read_csv("Trees/Tree")
Tree_Ed<-read_csv("Trees/Tree_Ed")
Tree_Ex<-read_csv("Trees/Tree_Ex")
Tree_Job<-read_csv("Trees/Tree_Job")
Tree_Site<-read_csv("Trees/Tree_Site")

Tree_Ag_skills<-read_csv("Trees/Tree_Ag_skills")
Tree_Ag_Ed<-read_csv("Trees/Tree_Ag_Ed")
Tree_Ag_Ex<-read_csv("Trees/Tree_Ag_Ex")
Tree_Ag_Job<-read_csv("Trees/Tree_Ag_Job")
Tree_Ag_Site<-read_csv("Trees/Tree_Ag_Site")

Tree_IT_skills<-read_csv("Trees/Tree_IT_skills")
Tree_IT_Ed<-read_csv("Trees/Tree_IT_Ed")
Tree_IT_Ex<-read_csv("Trees/Tree_IT_Ex")
Tree_IT_Job<-read_csv("Trees/Tree_IT_Job")
Tree_IT_Site<-read_csv("Trees/Tree_IT_Site")

Tree_Man_skills<-read_csv("Trees/Tree_Man_skills")
Tree_Man_Ed<-read_csv("Trees/Tree_Man_Ed")
Tree_Man_Ex<-read_csv("Trees/Tree_Man_Ex")
    ...
    sidebar = dashboardSidebar(
      sidebarMenu(
        ...
        menuItem(
          ...
          text = "Industry Built Amenities",
          icon = icon("map-marked-alt")
        ),
        menuItem(
          tabName = "humancapital",
          text = "Human Capital in Wythe County",
          icon = icon("map-marked-alt")
        ),
        menuItem(
          tabName = "jobs",
          text = "U.S. Jobs of Tomorrow",
          icon = icon("map-marked-alt")
        ),
        menuItem(
          tabName = "access",
          text = "Regional Education Access",
          icon = icon("map-marked-alt")
        ),
        menuItem(
          tabName = "conclusions",
        ...
        tabItems(
          tabItem(tabName = "overview",
                  fluidRow(
                    boxPlus(
                      ...
                      h2("Project Approach"),
                      p("Acemoglu and Autor (2012) demonstrate that investments in human capital have large effects on both the labor market and the economy. They build on work by Goldin and Katz (2007), who also contend that there is strong evidence that investments in human capital such as education have far reaching effects on the economy, public policy, and society. Our approach to studying future industry growth potential in Wythe  focused predominantly on human capital; to that end, we examined Wythe’s comparative ability to develop its population of workers to meet  workforce needs for the jobs of tomorrow, i.e., those jobs with the most promising outlook for employment and income growth."),
                      ...
                      p("[1] Image Source: https://en.wikipedia.org/wiki/National_Register_of_Historic_Places_listings_in_Wythe_County,_Virginia")
                    )
                  )),
          tabItem(tabName = "data",
                  fluidRow(
                    boxPlus(
                      ...
                      p("Waldorf, B. S., & Che, S. E. (2010). Spatial models of health outcomes and health behaviors: the role of health care accessibility and availability. In Progress in spatial analysis (pp. 339-362). Springer, Berlin, Heidelberg.")
                    )
                  )),
          ...
          tabItem(tabName = "humancapital",
                  fluidRow(
                    ...
                    p("First, the workforce in Wythe County is concentrated in the health, education, manufacturing, and retail industries. Outside of education and healthcare, most workers in Wythe are in the retail sector.  While these types of service jobs are important, it should be recognized that these are not jobs that are typically associated with high earning potential. Moreover, e-commerce is continuing to crowd out many of these types of businesses. From an economic growth perspective, it is therefore important to continue diversifying the industrial base. "),
          ...
          tabItem(tabName = "access",
                  fluidRow(
                    ...
                    p("An analysis of the trees (see previous tab) shows that many of the jobs in high growth careers are in information and technology. These jobs often require training in a host of critical areas, from customer support and client satisfaction to computer science and systems and electrical engineering. Aside from the heavy STEM focus in information technology, there is also a large need for people with skills in communication, education, and training, among others. Additionally, there are many high-paying jobs in the IT sector that do not require a college degree. IT jobs such as web developer, computer programmer, systems analyst, cybersecurity analyst, graphic designer, digital marketer, and telecommunications technician can be filled by individuals that have relevant skills but not necessarily a four-year degree. In such cases, local community colleges and high schools can help with IT skill development by providing courses and certification programs that are readily accessible to residents in Wythe County, particularly those without a four-year college degree."),
