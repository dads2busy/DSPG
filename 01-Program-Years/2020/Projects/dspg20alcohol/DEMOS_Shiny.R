...
#setwd("C:/Users/dgthomas/Documents/data_3")
library(DSPG)
...
Iowa_unemployment_data_selected_grouped <- Iowa_unemployment_data_selected %>%
  group_by(AREANAME, YEAR) %>%
  dplyr::summarise(mean_ratePerYear = sum(mean(UNEMPRATE)/12)) ##Initially called ddf_1
#summarise(mean_ratePerYear = sum(mean_rate)/12)
...
map_Data_Unemp <- merge(mapData,Iowa_unemployment_data_final_sel,by=c("county","year"))
#view(map_Data_Unemp)
#New column having volume sold per capita
map_Data_Unemp$Vol_solpercap = with(map_Data_Unemp, sumVolumeSold/population)
...
bins <- c(0,( mean(map_Data_Unemp$Vol_solpercap)-2*sd(map_Data_Unemp$Vol_solpercap)), (mean(map_Data_Unemp$Vol_solpercap)-1*sd(map_Data_Unemp$Vol_solpercap)), (mean(map_Data_Unemp$Vol_solpercap)),(mean(map_Data_Unemp$Vol_solpercap)+1*sd(map_Data_Unemp$Vol_solpercap)),(mean(map_Data_Unemp$Vol_solpercap)+2*sd(map_Data_Unemp$Vol_solpercap)), Inf)

bins_1 <- c(0,(mean(map_Data_Unemp$mean_ratePerYear)-2*sd(map_Data_Unemp$mean_ratePerYear)), (mean(map_Data_Unemp$mean_ratePerYear)-1*sd(map_Data_Unemp$mean_ratePerYear)), (mean(map_Data_Unemp$mean_ratePerYear)),(mean(map_Data_Unemp$mean_ratePerYear)+1*sd(map_Data_Unemp$mean_ratePerYear)),(mean(map_Data_Unemp$mean_ratePerYear)+2*sd(map_Data_Unemp$mean_ratePerYear)),Inf)

pal <- colorBin("Blues", domain = map_Data_Unemp$Vol_solpercap, bins = bins)
pal_1 <- colorBin("Blues", domain = map_Data_Unemp$mean_ratePerYear, bins = bins_1)
###########################Deepak's Portion#####################################
...
df1 = read.csv('data/Iowa_Liquor_Sales_Agg.csv', stringsAsFactors = FALSE)

#df1 = df %>% count(Date)
#df1 = dplyr::rename(df1, ds = Date = ds, y = x)

#df1$ds <- as.Date(df1$ds , format = "%m/%d/%Y") # not the right format
df1$ds <- as.Date(df1$ds , format = "%Y-%m-%d")
...
df2 = read.csv('data/Campus_Town_Agg.csv')
...
# terrible solution, but let's try to get this to work
