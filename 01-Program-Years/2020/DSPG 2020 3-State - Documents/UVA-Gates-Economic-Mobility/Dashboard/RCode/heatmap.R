...
ggplot(VOTE, aes(y=Questions, x=States, fill=factor(Scores))) +
  ...
  coord_equal() +
  theme_SDAD() +
  annotate("text", x=rep(c(1:3), length(unique(Questions))),
...
#Title of the policy domains (you will need to change y)
  annotate("text", x=2, y=9.5,label="Voting Accessibility", fontface=2, color="black") +
  annotate("text", x=2, y=9.5,label="Voting Accessibility", fontface=2, color="black") +
  annotate("text", x=2, y=5.5,label="Voter Registration", fontface=2, color="black") +
  annotate("text", x=2, y=5.5,label="Voter Registration", fontface=2, color="black") +
