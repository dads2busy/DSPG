...
covid19_calls = read_excel("COVID.xlsx")

iowacovid = read_excel("iowaconcern.xlsx")
...
covid19_calls = rename(covid19_calls, Date = `43891`, Counts = `2696`)
##Giving the name of the dataset from where it was taken
covid19_calls$Data = 'All Calls'
iowacovid = rename(iowacovid, Date = `43891`, Counts = `6`)
##Giving the name of the dataset from where it was taken

iowacovid$Data = 'Iowa Concern'
covid_merge = rbind(covid19_calls, iowacovid)
#covid19_calls_long = gather(covid19_calls, Number_of_Cases , factor_key = TRUE ) #Convert from wide to long format
#Renaming first column
...
sidebar <- dashboardSidebar(
  ...
  sidebarMenu(
    ...
    menuItem("COVID - 19", icon = icon("bar-chart-o"), tabName = "covid19")
    ...
    tabItem(tabName = "covid19",
            ...
            fluidRow(
              ...
              boxPlus(
                ...
                sidebar_start_open = FALSE,
                sidebar_content = tagList(
                  selectInput(inputId = "yearx", label = strong("Data"),
                              choices = unique(c$year),
                              selected = "2020")
                ),
                plotlyOutput(outputId = "covid_line_plot")
              )
            )
...
server <- function(input, output){
  ...
  output$covid_line_plot = renderPlotly({ggplot(covid_merge, aes(x=Date, y=Counts, group = Data))+
      geom_line(aes(linetype=Data))+
      geom_point(aes(color=Data))
    ggplot(covid_merge, aes(x=Date, y=Counts, group = Data))+
      geom_line(aes(linetype=Data))+
      geom_point(aes(color=Data))+theme_minimal()})
}
