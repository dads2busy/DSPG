match_cert = function(certificates, verbose = FALSE){
  ...
  do.call(rbind, lapply(seq_along(certificates), function(certificate){
    ...
    for(i in which(!recognized)){
      ...
      print(paste0("First Loop, iteration ",i)) #debug
...
## remove duplicate certificates and make a certificate-term matrix
sources <- onet[,c(2,5)]
colnames(sources) <- c("certificate","SOC")
sources = sources[!duplicated(sources[c(1,2)]),]
ctm = lma_weight(lma_dtm(sources$certificate, numbers = TRUE))
...
etm = lma_dtm(
  vapply(names(certifications), function(SOC) paste(SOC, certifications[[SOC]], collapse = ' '), ''),
  numbers = TRUE
)

# make versions of term for partial matching
#ELK question: is this stemming?
suffixes = "(?:'?s|ed|er|ing|ive)$" #may be some issues here, check it out. or may be a bad one to keep
